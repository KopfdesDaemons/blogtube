=== BlogTube ===
Contributors: Rico
Tags: blog, one-column, two-columns, right-sidebar, custom-colors, custom-menu, full-width-template, sticky-post, theme-options, threaded-comments, translation-ready
Requires at least: 5.0
Requires PHP: 7.0
Tested up to: 8.1
License: GNU General Public License v2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

== Description ==

This theme is designed for blogs. BlogTube is a theme inspired by a well-known video platform. It has an infinite scroll function.

== Installation ==

1. In your WordPress dashboard, navigate to "Appearance" > "Themes."
2. Click on "Add New" at the top of the page.
3. Click on "Upload Theme."
4. Choose the ZIP file of the BlogTube theme that you downloaded from the official WordPress Themes website.
5. Click on "Install Now."
6. Activate the BlogTube theme once the installation is complete.

== Changelog ==

    = 1.2.0 =
        - removed theme author website link

    = 1.1.0 =
        - chipmenu above the feed
        - home widget area

    = 1.0.1 =
        - load_posts() function
            - prefix
            - response without words if no posts can be loaded

    = 1.0.0 =
        - Initial release

== License ==

    = Images =

    Image for website icon, Copyright 2023 Henry
    License: CC0 1.0 Universal (CC0 1.0)
    License URL: https://creativecommons.org/publicdomain/zero/1.0/
    Source: https://openclipart.org/detail/343548/play

    Image for theme screenshot (Post Thumbnail), Copyright 2017 Kesäperuna
    License: Attribution-ShareAlike 4.0 International (CC BY-SA 4.0)
    License URL: https://creativecommons.org/licenses/by-sa/4.0/deed.en
    Image URL: https://commons.wikimedia.org/wiki/File:Linux_Mint_18.3_Cinnamon_Mint_Y.png

    Image for theme screenshot (Post Thumbnail), Copyright 2018 Rawpixel Ltd
    License: Attribution 2.0 Generic (CC BY 2.0)
    License URL: https://creativecommons.org/licenses/by/2.0/
    Source: https://www.flickr.com/photos/byrawpixel/39726357210/

    Image for theme screenshot (Post Thumbnail), Copyright 2020 Cloudtail the Snow Leopard
    License: Attribution-NonCommercial-NoDerivs 2.0 Generic (CC BY-NC-ND)
    License URL: https://creativecommons.org/licenses/by-nc-nd/2.0/
    Source: https://www.flickr.com/photos/blacktigersdream/50908648771

    Image for theme screenshot (Post Thumbnail), Copyright 2014 avalon20_(mac)
    License: Attribution-NoDerivs 2.0 Generic (CC BY-ND)
    License URL: https://creativecommons.org/licenses/by-nd/2.0/
    Source: https://www.flickr.com/photos/avalon20/47288095632

    Image for theme screenshot (Post Thumbnail), Copyright Tim Reckmann 
    License: Attribution 2.0 Generic (CC BY 2.0)
    License URL: https://creativecommons.org/licenses/by/2.0/
    Source: https://ccnull.de/foto/bitcoin-steigend/1091658

    Image for theme screenshot (Author Profile Picture),
    License: No Rights Reserved (CC=0)
    License URL: https://creativecommons.org/public-domain/cc0/
    Source: https://pxhere.com/en/photo/24303

    = Fonts =

    Google Fonts
    License: Open Font License
    License URL: https://scripts.sil.org/cms/scripts/page.php?site_id=nrsi&id=OFL
    Source: https://fonts.google.com/

    Font Awesome
    License: Font Awesome Free License
    license URI: https://fontawesome.com/license/free
    Source: https://fontawesome.com/start


BlogTube WordPress Theme, Copyright 2024 Rico Angenvoort
BlogTube is distributed under the terms of the GNU GPL